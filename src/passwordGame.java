...
public class PasswordGame {
    ...
    static int test(String password){
        ...
        //Rule 4: Password must contain "alphabet" (funny)
        if (!password.contains("alphabet") && !password.contains("Alphabet")) {
            return 4;
        }
        
        //Rule 5: What is light during the day but heavy at night (eyelids)
        if (!password.contains("eyelids") && !password.contains("Eyelids")) {
            return 5;
        }
        
        //Rule 6: What starts and ends with "E" but only has one letter in it? (envelope or eye)
        if (!password.contains("envelope") && !password.contains("Envelope") && !password.contains("eye") && !password.contains("Eye")) {
            return 6;
        }
        
        //Rule 7: Must include the best chess move in the position r1bqkb1r/ppp2ppp/2n2n2/3pp1N1/2B1P3/8/PPPP1PPP/RNBQK2R w KQkq - 0 1 (exd5)
        if (!password.contains("exd5")) {
            return 7;
        }
            
        //Rule 8: Must include the best atomic chess move in the same position (nxf7)
        if (!password.contains("nxf7")) {
            return 8;
        }
        
        //Rule 9: Must contain the name of Max Design Pro's little brother?
        if (!password.contains("jimmy") && !password.contains("Jimmy")) {
            return 9;
        }
        //Rule 10: Must contain the roman numeral for 500: 
